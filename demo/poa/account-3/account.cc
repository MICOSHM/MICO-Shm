...
Account_ptr Bank_stub::create( const char* _par_name )
{
...
Account_ptr
Bank_stub_clp::create( const char* _par_name )
{
  PortableServer::Servant _serv = _preinvoke ();
  if (_serv) {
    ...
    if (_myserv) {
      ...
      try {
      #endif
        __res = _myserv->create(_par_name);
      #ifdef HAVE_EXCEPTIONS
      }
      catch (...) {
        _myserv->_remove_ref();
  ...
  return Bank_stub::create(_par_name);
}

#endif // MICO_CONF_NO_POA

void Bank_stub::shutdown()
{
